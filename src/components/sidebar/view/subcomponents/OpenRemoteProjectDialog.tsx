...
function joinPosixPath(dir: string, name: string): string {
  if (dir === '/') {
    return `/${name}`;
  }
...
type OpenRemoteProjectDialogProps = {
  open: boolean;
...
export default function OpenRemoteProjectDialog({
  open,
  onOpenChange,
  serverId,
  onSuccess,
  t,
}: OpenRemoteProjectDialogProps) {
  ...
  const loadBrowse = useCallback(
    async (p?: string) => {
      ...
      setError(null);
      try {
        const res = await api.sshServers.browseRemoteDir(serverId, p);
  ...
  useEffect(() => {
    if (open) {
      ...
      setBrowse(null);
      setPathInput('');
      void loadBrowse(undefined);
    }
  }, [open, loadBrowse, serverId]);
  ...
  const onGo = () => {
    ...
    if (!tPath.startsWith('/')) {
      setError(t('projects.openRemotePathMustAbsolute'));
      return;
    }
  ...
  const onOpenHere = async () => {
    ...
    setError(null);
    try {
      const res = await api.sshServers.openClaudeProject(serverId, { projectPath: cwd });
      const j = (await res.json().catch(() => ({}))) as { error?: string };
      if (!res.ok) {
        setError(j.error || t('projects.openOnRemoteFailed'));
        return;
      }
      onOpenChange(false);
      onSuccess();
    } catch (e) {
      setError(e instanceof Error ? e.message : t('projects.openOnRemoteFailed'));
    } finally {
  ...
  return ReactDOM.createPortal(
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="open-remote-project-title"
      >
        <div className="flex items-center justify-between border-b border-border bg-muted/30 px-4 py-3">
          <h2 id="open-remote-project-title" className="text-base font-semibold text-foreground">
            {t('projects.openRemotePathDialogTitle')}
          </h2>
          <button
            type="button"
        ...
        <div className="space-y-3 p-4">
          <p className="text-xs text-muted-foreground leading-relaxed">{t('projects.openRemotePathDialogHint')}</p>
          <div className="flex gap-2">
            <Input
              ...
              placeholder={t('projects.openRemotePathPlaceholder')}
              className="font-mono text-xs"
              autoComplete="off"
            />
            <Button type="button" variant="secondary" onClick={onGo} disabled={browseLoading}>
              {t('projects.openRemotePathGo')}
            </Button>
          </div>
          <div className="flex items-center justify-between gap-2">
            <p className="min-w-0 flex-1 truncate font-mono text-xs text-foreground" title={browse ? browse.path : pathInput}>
              {browse ? browse.path : '—'}
            </p>
            <Button
              type="button"
                  ...
                  <li key={e.name}>
                    {e.isDirectory ? (
                      <button
                        ...
                        onClick={() => void loadBrowse(joinPosixPath(browse.path, e.name))}
                      >
        ...
        <div className="flex gap-3 border-t border-border bg-muted/20 px-4 py-3">
          ...
          >
            {openLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : t('projects.openRemoteOpenHere')}
          </Button>
        </div>
      </div>
    </div>,
